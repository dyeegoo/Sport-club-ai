...
import com.sportclubai.presentation.dashboard.StudentPanelState
import com.sportclubai.presentation.dashboard.StudentPanelViewModel
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { navController.navigate(Screen.MyNotifications.route) }) {
                        Icon(Icons.Default.Notifications, contentDescription = "Notifications")
                    }
                    IconButton(onClick = { navController.navigate(Screen.MySettings.route) }) {
...
@Composable
fun StudentHomeContent(data: StudentDashboardData, navController: NavController) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
        ...
        Row(
            ...
            StudentStatCard(
                ...
                modifier = Modifier.weight(1f),
                onClick = { navController.navigate(Screen.MyMessages.route) }
            )
        }

        // Training Plan
        Card(
            modifier = Modifier.fillMaxWidth().clickable { navController.navigate(Screen.MyTraining.route) }
        ) {
        ...
        // Next Class
        Card(
            modifier = Modifier.fillMaxWidth().clickable { navController.navigate(Screen.MySchedule.route) }
        ) {
            Column(modifier = Modifier.padding(16.dp)) {
        ...
        // Latest Payment
        Card(
            modifier = Modifier.fillMaxWidth().clickable { navController.navigate(Screen.MyPayments.route) }
        ) {
            Column(modifier = Modifier.padding(16.dp)) {
        ...
        // Belt Exam
        Card(
            modifier = Modifier.fillMaxWidth().clickable { navController.navigate(Screen.MyBeltProgress.route) }
        ) {
            Column(modifier = Modifier.padding(16.dp)) {
