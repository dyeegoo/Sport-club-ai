...
class AddStudentUseCase @Inject constructor(
    ...
    suspend operator fun invoke(student: Student, imageUri: Uri?): String {
        ...
        if (user.role != "owner" && user.role != "coach") {
            throw Exception("Unauthorized: Only owners and coaches can add students")
        }
        
        return studentRepository.addStudent(uid, student, imageUri)
    }
}
